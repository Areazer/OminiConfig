...
服务启动后访问：
- 🖥️ **GUI 界面**: http://localhost:8000 (Vue 3 + Tailwind CSS 可视化编辑器)
...
OminiConfig 内置了一个现代化的 Web GUI，基于 Vue 3 和 Tailwind CSS 构建，无需额外安装。

### 特性

- 🎨 **深色主题**: 极客风格暗色界面，保护眼睛
- 🌳 **树形表单**: 根据 JSON Schema 自动渲染多层嵌套表单
- 🔄 **实时同步**: SSE 实时推送配置变更，支持外部程序修改检测
- ⚡ **乐观锁**: 保存时自动处理并发冲突
- 📱 **响应式**: 支持桌面和移动设备

### 使用步骤

1. **启动服务**后访问 http://localhost:8000
2. **查看配置**: 自动加载 `app/settings.json` 并显示树形表单
3. **编辑配置**: 点击对应字段直接修改（支持字符串、数字、布尔值、对象、数组）
4. **保存配置**: 点击右上角"保存"按钮，自动携带 versionHash 防止冲突
5. **实时监控**: 顶部状态栏显示连接状态，外部修改时会弹出 Toast 提示

### 防冲突机制

当外部程序（如其他编辑器、脚本）修改了配置文件时：
- GUI 会检测到 Hash 不一致
- **不会**强制刷新您正在编辑的内容
- 显示 Toast 提示："配置已被外部程序修改，为防止覆盖，请点击刷新"
- 您可以选择立即刷新或稍后处理
